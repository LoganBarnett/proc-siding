  ...
  match output {
    ...
    Err(e) => {
      if e.kind() == std::io::ErrorKind::NotFound {
        eprintln!(
                    "Binary not found. Please build the project first with: cargo build -p proc-siding-cli"
                );
      }
...
fn test_version_flag() {
  ...
  match output {
    Ok(output) => {
      ...
      let stdout = String::from_utf8_lossy(&output.stdout);
      assert!(
        stdout.contains("proc-siding-cli"),
        "Expected version text to contain 'proc-siding-cli', got: {}",
        stdout
      );
    }
    Err(e) => {
